<?php
if (!class_exists('Ikreate_Fse_Welcome')) :

    class Ikreate_Fse_Welcome {
		 ...
		 *
		 */
		private static function ikreate_fse_welcome_redirect( $url ) {
        ...
        */
        public function __construct() {
            ...
            $this->tab_sections = array(
                'getting_started' => esc_html__('Getting Started', 'ikreate-fse'),
                'recommended_plugins' => esc_html__('Recommended Plugins', 'ikreate-fse'),
                'support' => esc_html__('Support', 'ikreate-fse')
            );
            ...
            add_action('wp_ajax_ikreate_fse_welcome_activate_plugin', array($this, 'activate_plugin'));

			add_action( 'wp_ajax_thim_core_installer', array( $this, 'ikreate_fse_welcome_ajax_install' ) );
			
            //add_action( 'admin_notices', array( $this, 'notice_install' ) );

			add_action( 'ikreate_fse_welcome_installer_step_start', array( $this, 'ikreate_fse_welcome_prepare_installation' ) );
			add_action( 'ikreate_fse_welcome_installer_step_install', array( $this, 'ikreate_fse_welcome_step_install' ) );
			add_action( 'ikreate_fse_welcome_installer_step_activate', array( $this, 'ikreate_fse_welcome_step_activate' ) );

        }

        /** Trigger Welcome Message Notification */
        public function admin_notice($hook) {
            $hide_notice = get_option('ikreate_fse_welcome_hide_notice');
            if (!$hide_notice) {
        ...
        public function admin_notice_content() {
            ...
            if ('appearance_page_ikreate_fse-welcome' === $screen->id || (isset($screen->parent_file) && 'plugins.php' === $screen->parent_file && 'update' === $screen->id) || 'theme-install' === $screen->id) {
                return;
            }

            $slug = $filename = 'elementor';
            ?>
            <div class="updated notice ikreate_fse-welcome-notice">
                <div class="ikreate_fse-welcome-notice-wrap">
                    <h2><?php esc_html_e('Congratulations!', 'ikreate-fse'); ?></h2>
                    <p><?php printf(esc_html__('%1$s is now installed and ready to use. You can start either by importing the ready made demo or get started by customizing it your self.', 'ikreate-fse'), esc_attr($this->theme_name)); ?></p>

                    <div class="ikreate_fse-welcome-info">
                        <div class="ikreate_fse-welcome-thumb">
                            <img src="<?php echo esc_url(get_stylesheet_directory_uri() . '/screenshot.png'); ?>" alt="<?php echo esc_attr__('Ikreate FSE Demo', 'ikreate-fse'); ?>">
                        </div>

                        <div class="ikreate_fse-welcome-import">
                            <h2><?php esc_html_e('Import Demo', 'ikreate-fse'); ?></h2>
                            <p><?php esc_html_e('Click below to install and activate the One Click Demo Importer Plugin, which helps you import multiple sample demos.', 'ikreate-fse'); ?></p>
                            <p><?php echo $this->ikreate_fse_welcome_setup_content(); ?></p>
                        </div>
                        
                        <div class="ikreate_fse-welcome-getting-started">
                            <h2><?php esc_html_e('Get Started', 'ikreate-fse'); ?></h2>
                            <p><?php printf(esc_html__('Here you will find all the necessary features about the Free and Premium themes, links, and information on how to use %s.', 'ikreate-fse'), esc_attr($this->theme_name)); ?></p>
                            <p><a href="<?php echo esc_url(admin_url('admin.php?page=ikreate_fse-welcome')); ?>" class="button button-primary"><span class="dashicons dashicons-admin-settings"></span> <?php esc_html_e('Go to Setting Page', 'ikreate-fse'); ?></a></p>
                        </div>
                    </div>
                    <a href="<?php echo wp_nonce_url(add_query_arg('ikreate_fse_welcome_hide_notice', 1), 'ikreate_fse_welcome_hide_notice_nonce', '_ikreate_fse_welcome_notice_nonce'); ?>" class="notice-close"><?php esc_html_e('Dismiss', 'ikreate-fse'); ?></a>
                </div>
            </div>
            <?php
        }
        ...
        public function welcome_screen() {
            ...
            <div class="welcome-wrap">
                <div class="welcome-main-content">
                    <?php require_once get_template_directory() . '/core/welcome/sections/header.php'; ?>
                    ...
                    <div class="welcome-section-wrapper">
                        ...
                        <div class="welcome-section <?php echo esc_attr($section); ?> clearfix">
                            <?php require_once get_template_directory() . '/core/welcome/sections/' . $section . '.php'; ?>
                        </div>
                    </div>
                </div>
            </div>
        ...
        public function welcome_styles_and_scripts($hook) {
            if ('theme-install.php' !== $hook) {
                $importer_params = array(
                    'installing_text' => esc_html__('Installing Demo Importer Plugin', 'ikreate-fse'),
                    'activating_text' => esc_html__('Activating Demo Importer Plugin', 'ikreate-fse'),
                    'importer_page' => esc_html__('Go to Demo Importer Page', 'ikreate-fse'),
                    'importer_url' => admin_url('themes.php?page=elementor'),
                    'error' => esc_html__('Error! Reload the page and try again.', 'ikreate-fse'),
                );
                wp_enqueue_style('ikreate_fse-welcome', get_template_directory_uri() . '/core/welcome/css/welcome.css', array());
                wp_enqueue_script('ikreate_fse-welcome', get_template_directory_uri() . '/core/welcome/js/welcome.js', array('plugin-install', 'updates'), '', true);
                wp_localize_script('ikreate_fse-welcome', 'importer_params', $importer_params);
            }
        }
        ...
        public function admin_footer_text($text) {
            ...
            if ('toplevel_page_ikreate_fse-welcome' == $screen->id) {
                $text = sprintf(esc_html__('Please leave us a %s rating if you like our theme . A huge thank you from ikreate themes in advance!', 'ikreate-fse'), '<a href="https://wordpress.org/support/theme/ikreate-fse/reviews/?filter=5#new-post" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>');
            }
        ...
        public function erase_hide_notice() {
            delete_option('ikreate_fse_welcome_hide_notice');
        }
		 ...
		 *
		 * @return ikreate_fse_welcome_Plugin
		 * @since 1.0.0
		 *
		 */
		private function ikreate_fse_welcome_get_thim_core() {
			if ( $this->plugin === null ) {
				$plugin = new ikreate_fse_welcome_Plugin();
		 ...
		 *
		 */
		private function ikreate_fse_welcome_get_link_step( $step = '' ) {
		 ...
		 * @since 1.0.0
		 */
		public function ikreate_fse_welcome_ajax_install() {
			$plugin = $this->ikreate_fse_welcome_get_thim_core();
		 ...
		 * @since 1.0.0
		 */
		private function ikreate_fse_welcome_setup_content() {
			
			$thim_core   = $this->ikreate_fse_welcome_get_thim_core();
			$status      = $thim_core->get_status();
			$step        = isset( $_REQUEST['step'] ) ? $_REQUEST['step'] : 'start';

			do_action( "ikreate_fse_welcome_installer_step_$step" ); 
		 ...
		 * @since 1.0.0
		 */
		public function ikreate_fse_welcome_prepare_installation() {
			...
			$link = $this->ikreate_fse_welcome_get_link_step( $step );
			
			if ( $status == 'inactive' ) {
				$import_class = 'button button-primary';
				$import_button_text = esc_html__('Activate Demo Importer Plugin', 'ikreate-fse');
			} elseif ( $status == 'active' ) {
				$import_class = 'button button-primary';
				$import_button_text = esc_html__('Go to Demo Importer Page', 'ikreate-fse');
				$link =  admin_url( '/themes.php?page=ikreatethemes-demo-importer' );
			} else {
				$import_class = 'thim-button-link button button-primary';
				$import_button_text = esc_html__('Install Demo Importer Plugin', 'ikreate-fse');	
			}
		 ...
		 * @since 1.0.0
		 */
		public function ikreate_fse_welcome_step_install() {

			$link = $this->ikreate_fse_welcome_get_link_step( 'activate' );
			...
			if ( $status != 'not_installed' ) {
				self::ikreate_fse_welcome_redirect( $link );
			}
            ...
            ?>
                <a class="button button-primary" data-href="<?php echo esc_url( $link ) ?>" href=<?php echo esc_url( $link ) ?>>
					<?php esc_html_e('Activate Demo Importer Plugin', 'ikreate-fse') ?>
				</a>
            <?php
			if ( $result ) {
				self::ikreate_fse_welcome_redirect( $link );
		 ...
		 * @since 1.0.0
		 */
		public function ikreate_fse_welcome_step_activate() {

			$plugin = $this->ikreate_fse_welcome_get_thim_core();

			if ( $plugin->activate( true ) || $plugin->is_active() ) {

				$this->ikreate_fse_welcome_redirect_dashboard();
		 ...
		 * @since 1.0.0
		 */
		private function ikreate_fse_welcome_redirect_dashboard() {

			$url = admin_url( 'themes.php?page=ikreatethemes-demo-importer' );

			self::ikreate_fse_welcome_redirect( $url );
		}

    }

    new Ikreate_Fse_Welcome();
